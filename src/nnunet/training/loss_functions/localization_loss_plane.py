...
class LogBarrierLoss(nn.Module):
    ...
    def forward(self, predict:torch.Tensor, y_onehot: torch.Tensor):
        ...
        '''
        predict = torch.transpose(predict, dim0=1, dim1=2)
        predict = predict.flatten(0, 1)
        y_onehot = torch.transpose(y_onehot, dim0=1, dim1=2)
        y_onehot = y_onehot.flatten(0, 1)
...
class LogBarrierLossF(nn.Module):
    def __init__(self, t_0=1, mu=0.05, bound_ratio=0.1):
        '''
        The foreground version log barrier
        '''
        super().__init__()
        self.t_0 = t_0
        self.mu = mu
    ...
    def forward(self, predict:torch.Tensor, y_onehot: torch.Tensor):
        ...
        '''
        predict = 1 - predict[:, 0]
        y_onehot = 1 - y_onehot[:, 0]

        sum_target = torch.mean(y_onehot.flatten(2), dim=-1)
        sum_predict = torch.mean(predict.flatten(2), dim=-1)
...
class LocalizationLoss(nn.Module):
    ...
    def forward(self, predict:torch.Tensor, y_onehot: torch.Tensor):
        '''
        Args:
            predict: size [N, channel, D, H, W...]
            target: size [N, channel, D, H, W...]
        '''
        # print('before transpose', predict.shape)
        predict = torch.transpose(predict, dim0=1, dim1=2)
        predict = predict.flatten(0, 1)

        # print('after transpose', predict.shape)
        y_onehot = torch.transpose(y_onehot, dim0=1, dim1=2)
        y_onehot = y_onehot.flatten(0, 1)

        dim_predict = torch.mean(predict[:, 1:], dim=-1)
        dim_label = torch.mean(y_onehot[:, 1:], dim=-1)
        loss = dis_calculation(predict=dim_predict, target=dim_label)

        dim_predict = torch.mean(predict[:, 1:], dim=-2)
        dim_label = torch.mean(y_onehot[:, 1:], dim=-2)
...
class LocalizationLossF(nn.Module):
    '''
    The localization loss using one dimension wasserstein distance for image segmentation
    Background is not calculated
    This is only the foreground version
    Args:
        smooth: the smooth value
    '''
    def __init__(self, smooth:float=1e-5):
    ...
    def forward(self, predict:torch.Tensor, y_onehot: torch.Tensor):
        ...
        # print('after transpose', predict.shape)
        y_onehot = 1 - y_onehot[:, 0]
        dim_predict = torch.mean(predict, dim=-1)
        dim_label = torch.mean(y_onehot, dim=-1)
        loss = dis_calculation(predict=dim_predict, target=dim_label)
        
        dim_predict = torch.mean(predict, dim=-2)
        dim_label = torch.mean(y_onehot, dim=-2)
        loss = loss + dis_calculation(predict=dim_predict, target=dim_label)
...
class CrossEntro(nn.Module):
    def __init__(self):
        super().__init__()
        self.eps = 1e-6
    
    def forward(self, predict, target):
        loss = -target*torch.log(torch.clamp(predict, min=self.eps))
        loss = torch.mean(loss)
        return loss


class CrossEntroM(nn.Module):
    def __init__(self):
        '''
        Determine the cross entropy based on the distribution to balance the data
        '''
        super().__init__()
        self.eps = 1e-6
    ...
    def forward(self, predict, target, mask):
        with torch.no_grad():
            total_sum = torch.sum(torch.mean(mask, dim=-1, keepdim=True), dim=-2, keepdim=True) 
            distribution_x = torch.cumsum(torch.mean(mask, dim=-1, keepdim=True), dim=-2) 
            distribution_x = distribution_x / (total_sum + self.eps)
            distribution_y = torch.cumsum(torch.mean(mask, dim=-2, keepdim=True), dim=-1) 
            distribution_y = distribution_y / (total_sum + self.eps)
            digit_mask = (torch.abs(distribution_x -0.5)<self.threshold) & (torch.abs(distribution_y -0.5)<self.threshold)

        loss = -target*torch.log(torch.clamp(predict, min=self.eps))
        loss = loss * digit_mask
        loss = torch.mean(torch.sum(loss, dim=[-1, -2, -3]) / (torch.sum(digit_mask, dim=[-1, -2, -3])+self.eps))
        return loss
...
class DC_and_LC_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        lc_loss = self.loc_loss(net_output, y_onehot)
        reg_loss  = self.regularization(net_output, y_onehot)
        ce_loss = self.ce(net_output, y_onehot, net_output)
        # dc_loss = self.dc(net_output, y_onehot, loss_mask=mask) if self.weight_dice != 0 else 0
    ...
    def set_dice(self, dice):
        ...
        '''
        self.dice_coeff = self.dice_coeff + dice / 100
