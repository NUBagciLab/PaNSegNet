...
class LinUNetEncoder(nn.Module):
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage, feat_map_mul_on_downscale,
        ...
        :param props:
        """
        super(LinUNetEncoder, self).__init__()
    ...
    def forward(self, x, return_skips=None):
        ...
        for s in self.stages:
            # print('shape for encode stage:', x.shape)
            x = s(x)
...
class LinUNetDecoder(nn.Module):
    def __init__(self, previous, num_classes, num_blocks_per_stage=None, network_props=None, deep_supervision=False,
        ...
        super(LinUNetDecoder, self).__init__()
        ...
        # spatial_or_self_attention = previous.spatial_or_self_attention
        ...
        for i, s in enumerate(np.arange(num_stages)[::-1]):
            ...
            if i<2:
                current_stage = SelfAttention3DBlock(in_feature1=features_skip, in_features2=features_skip, d_model=features_skip,
                                                     nhead=8, N=8)
            else:
                current_stage = SpatialAttention3DBlock(in_channel1=features_skip, in_channel2=features_skip, 
                                                        inter_channel=features_skip)

            self.att_stages.append(current_stage)
        ...
        self.stages = nn.ModuleList(self.stages)
        self.att_stages = nn.ModuleList(self.att_stages)
    ...
    def forward(self, skips):
        ...
        for i in range(len(self.tus)):
            # print('stage {s} skip', skips[i + 1].shape)
            # print('stage {s} x', x.shape)
            x = self.tus[i](x)
            middle = self.att_stages[i](skips[i + 1], x)
            # print('stage {s} skip after', x.shape)
            
            x = torch.cat((x, middle), dim=1)
            x = self.stages[i](x)
...
class LinUNet(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_blocks_per_stage_encoder, feat_map_mul_on_downscale,
        ...
        super(LinUNet, self).__init__()
        self.conv_op = props['conv_op']
        self.num_classes = num_classes
        self._deep_supervision = deep_supervision
        self.do_ds = deep_supervision

        self.encoder = LinUNetEncoder(input_channels, base_num_features, num_blocks_per_stage_encoder,
        ...
        self.decoder = LinUNetDecoder(self.encoder, num_classes, num_blocks_per_stage_decoder, props,
    ...
    def compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
        ...
        enc = LinUNetEncoder.compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
                                                                  num_modalities, pool_op_kernel_sizes,
                                                                  num_conv_per_stage_encoder,
                                                                  feat_map_mul_on_downscale, batch_size)
        dec = LinUNetDecoder.compute_approx_vram_consumption(patch_size, base_num_features, max_num_features,
