...
def check_dice_score(predict, target):
    dice = 1
    return dice

def compare_target(predict, target, target_class):
    dice_score = check_dice_score(predict, target)
    replace =  dice_score<0.9
    if replace:
        # return the mark value
        predict[predict==target_class] = 0
        predict[target==target_class] = target_class
    return predict, replace
