...
class LogBarrierLoss(nn.Module):
    ...
    def get_loss(self, z):
        ...
        if torch.any(torch.isinf(left)):
            print('isnan in z', torch.any(torch.isinf(z)))
            print('value of z', z)
            assert 1==2, 'now we know the problem'
    ...
    def forward(self, predict:torch.Tensor, target: torch.Tensor):
        ...
        for i in range(1, n_dim-2):
            if i == 1:
                ...
                dim_predict = torch.mean(dim_predict, dim=-1)
                dim_label = torch.mean(dim_label, dim=-1)
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
        ...
        if not square_dice:
            ...
            self.dc = SoftDiceLossSquared(apply_nonlin=None, **soft_dice_kwargs)
