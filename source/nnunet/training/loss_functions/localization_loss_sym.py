...
class DC_and_LC_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        lc_loss1 = self.loc_loss(net_output, y_onehot)
        lc_loss2 = self.loc_loss(1-net_output, 1-y_onehot)
        reg_loss  = self.regularization(net_output, y_onehot)
        # ce_loss = self.ce(net_output, target.squeeze(1))
        ...
        if self.aggregate == "sum":
            result = lc_loss1 + lc_loss2 + 0.1*reg_loss
        else:
